...
Copy the `public/scripts/extensions/third-party/le-eternalism` folder into your SillyTavern installation (same path), then restart SillyTavern (or use "Install extension from Git repo" in the Extensions menu).
...
- **Stage 1/2/3 prompts** — edit these freely; the extension just wires them. Stage 1 is two separate fields (system prompt 1 + system prompt 2) that are concatenated after the chat context.
...
- Prompts are placeholder text — replace them with the real ones.
