...
1. **Stage 1 — Scene analysis.** The request is sent as two separate system messages (*system prompt 1*: analysis commands, *system prompt 2*: thinking checklist) with the chat history (only `name: message` lines, no persona/character card/world info/preset prompts) as a user message. The AI replies with `[include: Name]` / `[exclude: Name]` directives (comma-separated names allowed).
