...
1. **Stage 1 — Scene analysis.** The request is assembled as: chat history (only `name: message` lines, no persona/character card/world info/preset prompts), then *system prompt 1* (analysis commands), then *system prompt 2* (thinking checklist). The AI replies with `[include: Name]` / `[exclude: Name]` directives (comma-separated names allowed).
